...
def analyze_all_endpoints(
    endpoints=None, pause=1, parameter_store=None, output_file=None
):
    """
    ...
    for name, endpoint_class in endpoints.items():
        ...
        try:
            ...
            if name in parameter_store.parameters and parameter_store.parameters[
                name
            ].get("status") in ["success", "deprecated"]:
...
def main():
    ...
    parser.add_argument(
        "--pause", type=float, default=1, help="Pause between API calls in seconds"
    )
    ...
    if args.endpoint:
        ...
        result = analyze_endpoint_with_retry(
            args.endpoint, retry_attempts=args.retry, pause=args.pause
        )
    ...
    if args.all:
        ...
        results = analyze_all_endpoints(
            endpoints=endpoints, pause=args.pause, output_file=args.output
        )

        # Print summary
        success_count = sum(1 for r in results.values() if r["status"] == "success")
        deprecated_count = sum(
            1 for r in results.values() if r["status"] == "deprecated"
        )
