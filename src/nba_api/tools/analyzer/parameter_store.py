...
class ParameterStore:
    ...
    def update_from_analysis(self, endpoint_name, analysis_result):
        ...
        for param in analysis_result.get("required_parameters", []):
            if param in self.parameters.get(endpoint_name, {}).get(
                "required_parameters", {}
            ):
                # Keep existing value
                required_params[param] = self.parameters[endpoint_name][
                    "required_parameters"
                ][param]
            elif (
                endpoint_name in self.missing_required_parameters
                and param in self.missing_required_parameters[endpoint_name]
            ):
                # Use value from fallback
                required_params[param] = self.missing_required_parameters[
                    endpoint_name
                ][param]
            else:
        ...
        self.parameters[endpoint_name]["nullable_parameters"] = analysis_result.get(
            "nullable_parameters", []
        )
        self.parameters[endpoint_name]["parameter_patterns"] = analysis_result.get(
            "parameter_patterns", {}
        )
        self.parameters[endpoint_name]["last_validated_date"] = analysis_result.get(
            "last_validated_date", ""
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Number of endpoints with fallback configs: {len(store.missing_required_parameters)}"
    )
